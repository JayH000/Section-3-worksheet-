...
def probabilistic_turing_machine_multiplication(a: str, b: str):
    # Implements a probabilistic Turing Machine for binary multiplication.
    ...
    with open(filename, 'w') as f:
        for step in itertools.count():
            ...
            # Introduce nondeterministic movement of head
            if random.random() < 0.5:  # 50% chance to move right
                if head_position < tape_length - 1:
                    head_position += 1
            else:  # 50% chance to jump ahead probabilistically
                jump = random.randint(1, min(3, tape_length - head_position - 1))
                head_position += jump
...
def compute_statistics():
    ...
    for L_a in range(2, size + 1):
        for L_b in range(2, size + 1):
            ...
            for _ in range(10):  # Test 10 random binary pairs of the same length
                ...
                steps = probabilistic_turing_machine_multiplication(a, b)
    ...
    plt.title('2D Heatmap of Probabilistic Computation Complexity ⟨n⟩')
    plt.savefig("complexity_heatmap_probabilistic.png")
    plt.show()
